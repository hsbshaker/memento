  ...
  return (
    <header className="relative z-40 bg-transparent">
      <div className="md:hidden">
        <div className="flex h-14 w-full min-w-0 items-center gap-3 border-b border-border bg-surface/80 px-4 backdrop-blur-md">
          {showNav ? (
            <button
              type="button"
              ...
              className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-border bg-surface text-foreground transition-colors hover:bg-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"
          ...
          <p className="min-w-0 truncate text-sm font-semibold tracking-tight text-foreground">{mobilePageTitle}</p>
        </div>
        ...
        >
          ...
          <aside
            ...
            className={cn(
              "absolute left-0 top-0 h-full w-[80%] max-w-xs border-r border-border bg-surface-raised p-4 shadow-2xl backdrop-blur-md transition-transform duration-200 ease-out",
              mobileDrawerOpen ? "translate-x-0" : "-translate-x-full",
            )}
          >
            <div className="mb-4 flex items-center justify-between">
              <span className="text-lg font-semibold tracking-tight text-foreground">Memento</span>
              <button
                type="button"
                onClick={() => setMobileDrawerOpen(false)}
                  ...
                  return (
                    <div key={tab.id} className="space-y-1">
                      <Link
                        ...
                        className={cn(
                          "block rounded-xl px-3 py-2.5 text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus",
                          tab.comingSoon
                      ...
                      {comingSoonTab === tab.id ? <p className="px-3 text-xs text-accent">Coming soon</p> : null}
                    </div>
                  );
                })}
              </nav>
                ...
                return (
                  <div key={tab.id} className="relative">
                    <Link
                      ...
                      onKeyDown={(event) => handleComingSoonAction(tab, event)}
                      className={cn(
                        "relative inline-flex items-center py-1 text-sm transition-colors duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus",
                        tab.comingSoon
                          ? "cursor-not-allowed text-subtle-foreground hover:text-subtle-foreground"
                          : isActive
                            ? "text-foreground"
                    ...
                    >
                      ...
                      <span
                        aria-hidden
                        className={cn(
                          "pointer-events-none absolute -bottom-1 left-0 h-0.5 rounded-full bg-accent transition-all duration-200 ease-out",
                    ...
                    {showComingSoon ? (
                      <div
                        id={tooltipId}
                        role="status"
                        className="absolute left-1/2 top-[calc(100%+8px)] z-50 -translate-x-1/2 whitespace-nowrap rounded-md border border-border bg-surface-raised px-2.5 py-1.5 text-xs text-foreground shadow-md backdrop-blur-sm"
                      >
                        Coming soon
                      </div>
                    ) : null}
