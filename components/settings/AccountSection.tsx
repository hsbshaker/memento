...
export function AccountSection({ email }: AccountSectionProps) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-medium tracking-[0.22em] text-muted-foreground uppercase">Account</p>
      <div className="flex items-center justify-between gap-4 rounded-xl border border-border bg-surface px-4 py-3.5">
        <div>
          <p className="text-xs text-muted-foreground">Signed in as</p>
          <p className="mt-0.5 text-sm font-medium text-foreground">{email ?? "Unknown email"}</p>
        </div>
