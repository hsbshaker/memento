-- Rewrite bootstrap RPC to match the v2 schema:
--   - benefits now carry card_id directly (no card_benefits junction table)
--   - user_benefits is now keyed by user_card_id (user_cards.id), not user_id
...
begin
  ...
  -- Resolve the user_cards row for this user + card
